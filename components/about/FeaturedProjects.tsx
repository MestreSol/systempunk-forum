...
const featuredProjects: FeaturedProject[] = [
  {
    ...
    icon: ({ className }) => <div className={`w-12 h-12 bg-lime-500/20 rounded-lg flex items-center justify-center ${className}`}>🎮</div>,
  ...
  },
  {
    ...
    icon: ({ className }) => <div className={`w-12 h-12 bg-cyan-500/20 rounded-lg flex items-center justify-center ${className}`}>💻</div>,
  ...
  },
  {
    ...
    icon: ({ className }) => <div className={`w-12 h-12 bg-purple-500/20 rounded-lg flex items-center justify-center ${className}`}>📱</div>,
      ...
      <CardContent>
        <div className="grid md:grid-cols-3 gap-6">
          {featuredProjects.map((project, index) => (
            <div key={index} className="bg-zinc-800 rounded-lg p-4 hover:bg-zinc-750 transition-colors">
              <project.icon className="mb-4" />
              <h3 className="text-lg font-semibold text-white mb-2">{project.name}</h3>
              <p className="text-zinc-400 text-sm mb-4">{project.description}</p>
              <div className="flex items-center gap-2">
                {getStatIcon(project.stats.type)}
                <span className="text-sm text-zinc-300">{project.stats.value}</span>
              </div>
            </div>
          ))}
        </div>
