...
import { mockArticles, categoryOptions, type NewsArticle } from "@/lib/data/newsData";
...
export default function NewsPage() {
  ...
  useEffect(() => {
    ...
    const loadArticles = async () => {
      try {
        ...
        setFilteredArticles(mockArticles.filter(article => article.status === 'published'));
        setIsLoading(false);
      } catch (error) {
      ...
      <div className="min-h-screen bg-zinc-950 text-white">
        <div className="container mx-auto px-6 py-12">
          <div className="flex items-center justify-center py-12">
            <div className="animate-pulse text-lime-400">Carregando notícias...</div>
          </div>
        </div>
      </div>
    );
  ...
  const featuredArticle = filteredArticles.find(article => article.status === 'published');
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      <div className="container mx-auto px-6 py-12">
        {/* Header */}
        <div className="mb-12">
          <h1 className="text-4xl font-bold text-lime-400 mb-4">Notícias SystemPunk</h1>
            ...
            <div className="flex gap-2">
              ...
              {selectedStatus !== "published" && (
                <Badge variant="outline" className="border-yellow-500 text-yellow-400">
                  {selectedStatus === "all" ? "Todos os status" : selectedStatus === "draft" ? "Rascunhos" : "Arquivados"}
                </Badge>
              )}
              {searchTerm && (
        ...
        {otherArticles.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-zinc-500">Tente ajustar os filtros ou termos de busca.</p>
          </div>
        ) : (
          <>
